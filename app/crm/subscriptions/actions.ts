...
export async function createSubscription(formData: FormData) {
  ...
  if (!UUID_REGEX.test(customerId) || !serviceName || !/^\d{4}-\d{2}-\d{2}$/.test(nextDueOn) || !Number.isInteger(frequencyMonths) || frequencyMonths < 1) {
    redirect("/crm/subscriptions?new=1&error=invalid");
  }
  ...
  try {
    ...
    redirect("/crm/subscriptions?new=1&error=storage");
  }
  ...
  redirect("/crm/subscriptions?created=1");
}
...
export async function advanceSubscription(formData: FormData) {
  ...
  if (
    !UUID_REGEX.test(id) ||
    !/^\d{4}-\d{2}-\d{2}$/.test(currentDue) ||
    !Number.isInteger(frequencyMonths) ||
    frequencyMonths < 1
  ) {
    redirect("/crm/subscriptions?error=invalid");
  }
  ...
  try {
    ...
    redirect("/crm/subscriptions?error=storage");
  }

  revalidatePath("/crm/subscriptions");
  redirect("/crm/subscriptions?updated=1");
}
