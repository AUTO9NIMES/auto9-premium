...
-- Review Request Email Delivery Guard.
--
-- Reliable Outbox Worker V1 delivers review.requested.v1 by email.
-- Prevent new review requests from creating permanently undeliverable
-- outbox events when the customer has no usable email address.
--
-- The idempotent replay path intentionally remains unchanged.
...
begin
  ...
  -- review.requested.v1 is delivered by the email worker introduced with
  -- Reliable Outbox Worker V1. Do not create a durable delivery event that
  -- has no usable email recipient. Existing idempotent review requests still
  -- return through the replay path above without being revalidated here.
  if nullif(trim(v_customer.email), '') is null then
    raise exception using
      errcode = '23514',
      message = 'Customer email is required to request a review';
  end if;
