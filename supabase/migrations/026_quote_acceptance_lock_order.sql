...
declare
  ...
  v_did_accept_quote boolean := false;
...
begin
  ...
  if v_quote.status in ('DRAFT', 'SENT') then
    select *
      into v_lead
      from public.leads
     where business_id = p_business_id
  ...
  if v_quote.status = 'ACCEPTED' then
    select a.*
      ...
      join public.jobs j
        on j.business_id = a.business_id
       and j.id = a.job_id
     where j.business_id = p_business_id
       and j.quote_id = p_quote_id
     for update of a;
  ...
  if v_quote.status = 'ACCEPTED' then
    ...
    if v_lead.lifecycle_status not in ('NEW', 'QUALIFIED', 'CONTACTED', 'QUOTE_SENT') then
      raise exception using
        errcode = '23514',
        message = format('Lead status %s cannot accept a quote', v_lead.lifecycle_status);
    end if;
  ...
  if v_requested_at is not null then
    if v_quote.status = 'ACCEPTED' and not v_has_existing_appointment then
      select *
        into v_existing_appointment
  ...
  if not v_did_accept_quote then
