create table if not exists public.review_requests (
  ...
  constraint fk_review_requests_business
    foreign key (business_id) references public.businesses (id)
    on delete restrict
    on update cascade,
  constraint fk_review_requests_job
    foreign key (business_id, job_id)
    references public.jobs (business_id, id)
    on delete restrict
    on update cascade,
  constraint ux_review_requests_business_job
    unique (business_id, job_id),
  constraint ux_review_requests_business_idempotency_key
