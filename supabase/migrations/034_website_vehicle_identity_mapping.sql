...
-- Exact website vehicle identity, separate from the pricing category.
-- Recreate the canonical 029 RPC; changing argument types/count creates a
-- different PostgreSQL function identity, not an in-place replacement.
-- Remove the old 22-argument signature without CASCADE so unexpected
-- dependencies abort this transaction instead of being silently removed.
...
begin
  ...
  -- Keep historical replay semantics. Only the two existing free-form
  -- sources may omit brand; their complete vehicle name is passed as model.
  if nullif(trim(coalesce(p_vehicle_model, '')), '') is null
     or (
       nullif(trim(coalesce(p_vehicle_brand, '')), '') is null
       and coalesce(p_source, 'website') not in (
         'website_pro_booking', 'website_special_request'
       )
     ) then
    raise exception using
      errcode = '22023',
      message = 'vehicle_model is required; vehicle_brand is required outside legacy intake';
  end if;
...
-- PostgREST resolves JSON argument names against its cached RPC signature.
