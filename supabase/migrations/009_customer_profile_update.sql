...
begin
  if p_business_id is null or p_customer_id is null then
    raise exception using
      errcode = '22023',
      message = 'business_id and customer_id are required';
  end if;
  ...
  if v_email is not null then
    select id into v_existing_identifier
      from public.customers
     where business_id = p_business_id
       and id <> p_customer_id
...
revoke all on function public.update_customer_profile(uuid, uuid, text, text, text, text, text, text, text)
  from public, anon, authenticated;

grant execute on function public.update_customer_profile(uuid, uuid, text, text, text, text, text, text, text)
  to service_role;
