...
begin
  ...
  -- Cancellation is a terminal business action and must remain possible even
  -- after booking or while a job is in progress. All non-cancellation
  -- transitions keep the original canonical lifecycle restrictions.
  if v_target_status <> 'CLOSED_LOST' then
    if v_lead.lifecycle_status = 'NEW' then
      ...
      if v_target_status <> 'CONTACTED' then
        raise exception using
          errcode = '23514',
          message = format('Invalid lead transition: %s -> %s', v_lead.lifecycle_status, v_target_status);
      end if;
    elsif v_lead.lifecycle_status = 'CONTACTED' then
      if v_target_status <> 'QUOTE_SENT' then
        raise exception using
          errcode = '23514',
          message = format('Invalid lead transition: %s -> %s', v_lead.lifecycle_status, v_target_status);
      end if;
